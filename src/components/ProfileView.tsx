  ...
  return (
    <div className="max-w-2xl space-y-4 sm:space-y-5">
      ...
      {statsLoading ? (
        ...
        <div className="bg-white border border-slate-200 p-5 sm:p-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            <div>
              <div className="text-2xl font-bold text-slate-900">{stats.totalTSS}</div>
              <div className="text-xs text-slate-400 mt-0.5">Total TSS</div>
            </div>
          </div>

          {Object.keys(stats.disciplineCounts).length > 0 && (
            <div className="border-t border-slate-100 pt-4">
              <div className="text-xs text-slate-400 font-medium uppercase tracking-wide mb-2">By Discipline</div>
              <div className="flex flex-wrap gap-2">
                {Object.entries(stats.disciplineCounts).map(([d, count]) => (
                  <div key={d} className={`px-2.5 py-1 text-xs font-semibold rounded-sm ${disciplineColors[d] ?? 'bg-slate-100 text-slate-600'}`}>
      ...
      <div className="bg-white border border-slate-200 p-5 sm:p-6">
        ...
        <button
          onClick={signOut}
          className="flex items-center gap-2 px-4 py-2.5 border border-red-200 text-red-600 text-sm font-medium hover:bg-red-50 hover:border-red-300 transition"
        >
